...
export const LivePreviewCard = (
  ...
  const { data } = useBackend<CustomMarkingDesignerData>(context);
  const [showSizeOptions, setShowSizeOptions] = useLocalState<boolean>(
    context,
    `previewSizeOptions-${data.state_token || 'session'}`,
    false
  );
  return (
    <Section
      ...
      <Flex align="center" wrap gap={0.5} mb={1} ml={0.5}>
        ...
        <Button
          className={CHIP_BUTTON_CLASS}
          icon="ruler-combined"
          selected={showSizeOptions}
          aria-label="Toggle size settings in preview"
          tooltip="Show or hide size, Odd/Even centering, and Fuzzy scaling in the preview."
