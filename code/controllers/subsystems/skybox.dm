	...
	if(global.using_map.use_overmap && settings.use_overmap_details)
		...
		if(istype(O))
			...
			if(isbelly(O.loc)) // Teehee
				var/obj/belly/ourbelly = O.loc
				base.icon = 'icons/skybox/skybox_rs.dmi'
				base.icon_state = "internal"
				if(ourbelly.belly_fullscreen_color)
					base.color = ourbelly.belly_fullscreen_color
				else
					base.color = "#7c3131"
			//VOREStation Add End
			//RS ADD
