...
/zone/proc/process_fire()
	var/datum/gas_mixture/burn_gas = air.remove_ratio(GLOB.vsc.fire_consuption_rate, fire_tiles.len) // RS Edit: vsc global fix (Lira, March 2026)
	...
	for(var/direction in cardinal)
		...
		if(istype(enemy_tile))
			if(my_tile.open_directions & direction) //Grab all valid bordering tiles
				...
				//Spread the fire.
				if(prob( 50 + 50 * (firelevel/GLOB.vsc.fire_firelevel_multiplier) ) && my_tile.CanPass(src, enemy_tile) && enemy_tile.CanPass(src, my_tile)) // RS Edit: vsc global fix (Lira, March 2026)
...
/obj/fire/proc/fire_color(var/env_temperature)
	var/temperature = max(4000*sqrt(firelevel/GLOB.vsc.fire_firelevel_multiplier), env_temperature) // RS Edit: vsc global fix (Lira, March 2026)
...
/datum/gas_mixture/proc/zburn(zone/zone, force_burn, no_check = 0)
	...
	if((temperature > PHORON_MINIMUM_BURN_TEMPERATURE || force_burn) && (no_check ||check_recombustability(zone? zone.fuel_objs : null)))
		...
		var/gas_firelevel = calculate_firelevel(gas_fuel, total_oxidizers, reaction_limit, volume*group_multiplier) / GLOB.vsc.fire_firelevel_multiplier // RS Edit: vsc global fix (Lira, March 2026)
		...
		var/liquid_firelevel = calculate_firelevel(liquid_fuel, total_oxidizers, reaction_limit, 0) / GLOB.vsc.fire_firelevel_multiplier // RS Edit: vsc global fix (Lira, March 2026)
		...
		temperature = (starting_energy + GLOB.vsc.fire_fuel_energy_release * (used_gas_fuel + used_liquid_fuel)) / heat_capacity() // RS Edit: vsc global fix (Lira, March 2026)
		update_values()
...
/datum/gas_mixture/proc/check_combustability(obj/effect/decal/cleanable/liquid_fuel/liquid=null)
	. = 0
	for(var/g in gas)
		if(gas_data.flags[g] & XGM_GAS_OXIDIZER && QUANTIZE(gas[g] * GLOB.vsc.fire_consuption_rate) >= 0.1) // RS Edit: vsc global fix (Lira, March 2026)
			. = 1
			break
	...
	. = 0
	for(var/g in gas)
		if(gas_data.flags[g] & XGM_GAS_FUEL && QUANTIZE(gas[g] * GLOB.vsc.fire_consuption_rate) >= 0.005) // RS Edit: vsc global fix (Lira, March 2026)
			. = 1
			break

//returns a value between 0 and GLOB.vsc.fire_firelevel_multiplier || RS Edit: vsc global fix (Lira, March 2026)
/datum/gas_mixture/proc/calculate_firelevel(total_fuel, total_oxidizers, reaction_limit, gas_volume)
	...
	if(total_combustables > 0)
		...
		firelevel = GLOB.vsc.fire_firelevel_multiplier * mix_multiplier * damping_multiplier // RS Edit: vsc global fix (Lira, March 2026)

	return max( 0, firelevel)


/mob/living/proc/FireBurn(var/firelevel, var/last_temperature, var/pressure)
	var/mx = 5 * firelevel/GLOB.vsc.fire_firelevel_multiplier * min(pressure / ONE_ATMOSPHERE, 1) // RS Edit: vsc global fix (Lira, March 2026)
