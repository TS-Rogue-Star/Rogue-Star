...
Class Procs:
	...
	flow(list/movable, differential, repelled)
		...
		A check against GLOB.vsc.lightest_airflow_pressure should generally be performed before calling this. || RS Edit: vsc global fix (Lira, March 2026)
...
/connection_edge/proc/flow(list/movable, differential, repelled)
	for(var/i = 1; i <= movable.len; i++)
		...
		if(M.last_airflow > world.time - GLOB.vsc.airflow_delay) continue // RS Edit: vsc global fix (Lira, March 2026)
		if(M.airflow_speed) continue

		//Check for knocking people over
		if(ismob(M) && differential > GLOB.vsc.airflow_stun_pressure) // RS Edit: vsc global fix (Lira, March 2026)
...
/connection_edge/zone/tick()
	...
	if(abs(differential) >= GLOB.vsc.airflow_lightest_pressure) // RS Edit: vsc global fix (Lira, March 2026)
		var/list/attracted
		var/list/repelled
		if(differential > 0)
