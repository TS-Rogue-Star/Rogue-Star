...
//Regular jobs
	...
	//JOBBAN'S INNARDS
		...
		if(L.can_centcom_reply())
			...
			to_chat(L, "<span class='info'>Please stand by for a message from <b><font color='blue'>Central Command</font></b>.</span>")
			to_chat(L, "<span class='info'>Message as follows.</span>")
			to_chat(L, "<span class='notice'>[input]</span>")
			to_chat(L, "<span class='info'>End of transmission.</span>")
		else
		...
		if(!input)
			message_admins("[key_name_admin(usr)] has cancelled their reply to Syndicate 5Emergency message.")
			return
