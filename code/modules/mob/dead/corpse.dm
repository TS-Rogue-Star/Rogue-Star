...
/obj/effect/landmark/mobcorpse
	...
	/// list(name of ear, color of ear, color of ear, ...). // RS EDIT START (Port of VS PR#16513 'Adds a second ear slot.')
	/// Color is optional, each position after the name is a color channel from 1 to n.
	var/list/ear_secondary_type // RS EDIT END (Port of VS PR#16513 'Adds a second ear slot.')
...
/obj/effect/landmark/mobcorpse/proc/createCorpse() //Creates a mob and checks for gear in each slot before attempting to equip it.
	...
	if(ear_type && ear_type.len)
		if(ear_type[1] in ear_styles_list)
			...
			M.update_hair()
	// handle secondary ears // RS EDIT START (Port of VS PR#16513 'Adds a second ear slot.')
	if(length(ear_secondary_type) && (ear_secondary_type[1] in global.ear_styles_list))
		...
		if(length(ear_secondary_type) > 1)
			M.ear_secondary_colors = ear_secondary_type.Copy(2, min(length(GLOB.fancy_sprite_accessory_color_channel_names), length(ear_secondary_type)) + 1) // RS EDIT END (Port of VS PR#16513 'Adds a second ear slot.')
