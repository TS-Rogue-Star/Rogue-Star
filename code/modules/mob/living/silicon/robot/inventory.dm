...
/mob/living/silicon/robot/proc/get_active_modules() //RS Edit: More elequent way of doing this
	return list(module_state_1, module_state_2, module_state_3) //RS Edit
...
/mob/living/silicon/robot/proc/has_active_type(var/type_to_compare, var/explicit = FALSE)
	var/list/active_modules = get_active_modules() //RS Edit
	if(is_type_in_modules(type_to_compare, active_modules, explicit)) //RS Edit. Changes from is_type_in_list to is_type_in_modules
		return TRUE
	return FALSE

///Searches through a provided list to see if we have a module that is in that list.  (RS Edit Start)
...
/mob/living/silicon/robot/proc/activate_module(var/obj/item/O)
	...
	if(!module_state_1)
		...
		if(istype(module_state_1,/obj/item/borg/sight))
			sight_mode |= module_state_1:sight_mode
		update_icon() //RS EDIT Moved from life.dm to here
