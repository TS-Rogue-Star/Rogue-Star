	...
	if(light_toggled)
		if(assembly)
			if(istype(assembly,/obj/item/device/electronic_assembly/clothing))
				var/obj/item/device/electronic_assembly/clothing/A = assembly
				A.clothing.set_light(l_range = light_brightness, l_power = light_brightness, l_color = light_rgb)
			else assembly.set_light(l_range = light_brightness, l_power = light_brightness, l_color = light_rgb)
	else
		if(assembly)
			if(istype(assembly,/obj/item/device/electronic_assembly/clothing))
				var/obj/item/device/electronic_assembly/clothing/A = assembly
				A.clothing.set_light(0)
			else assembly.set_light(0)
