	...
	if(href_list["digitigrade"])
		...
		*/ //VOREStation Removal End
		...
		switch(category_choice)
			...
			if("Arms and Legs")
				marking_category = pref.body_markings.Copy() ^ pref.get_available_styles(global.body_marking_limbs).Copy()
			if("Bandages and Scars")
				marking_category = pref.body_markings.Copy() ^ pref.get_available_styles(global.body_marking_addons).Copy()
			if("Skin and Panels")
				marking_category = pref.body_markings.Copy() ^ pref.get_available_styles(global.body_marking_skintone).Copy()
			if("Teshari Specific")
				marking_category = pref.body_markings.Copy() ^ pref.get_available_styles(global.body_marking_teshari).Copy()
			if("Vox Specific")
				marking_category = pref.body_markings.Copy() ^ pref.get_available_styles(global.body_marking_vox).Copy()
			if("Augmentations")
				marking_category = pref.body_markings.Copy() ^ pref.get_available_styles(global.body_marking_augment).Copy()
