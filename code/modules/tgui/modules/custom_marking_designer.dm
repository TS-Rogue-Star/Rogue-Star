...
/datum/tgui_module/custom_marking_designer/proc/build_static_species_part_order(datum/species/species)
	if(!istype(species) || !islist(species.has_limbs))
		return null
	var/list/order = list()
	for(var/part in BP_ALL)
		if(part in species.has_limbs)
			order += part
	for(var/part in species.has_limbs)
		if(!(part in order))
			order += part
	return order
...
/datum/tgui_module/custom_marking_designer/proc/build_static_species_body_preview_sources(species_id, preview_icon_base = null, gender_suffix = "m", digitigrade = FALSE)
	...
	for(var/dir in dirs)
		...
		if(!part_assets.len)
			continue
		var/list/resolved_part_order = part_order.Copy()
