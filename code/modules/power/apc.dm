...
/obj/machinery/power/apc/tgui_data(mob/user)
	var/list/data = list(
		...
		"siliconUser" = siliconaccess(user) || (isobserver(user) && is_admin(user)), //I add observer here so admins can have more control, even if it makes 'siliconUser' seem inaccurate. //RS Port Chomp PR 8077 |  //CHOMPEdit borg access
...
/obj/machinery/power/apc/tgui_act(action, params)
	...
	if(siliconaccess(usr) || action == "nightshift") //RS Port Chomp PR 8077 | CHOMPEdit borg access
