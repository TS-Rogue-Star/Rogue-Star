...
// Variant where max_amount is a per-material quantity.
/datum/component/material_container/per_material_silos
...
/datum/component/material_container
	...
	var/max_amount 					// TODO
	var/sheet_type					// What?
	var/list/materials				// Currently held materials (material_id -> quantity)
	var/show_on_examine				// If true, will print material amounts on examine.
	var/disable_attackby
	var/list/allowed_typecache	// populated on init
...
/**
* mat_list - list of starting materials (material_id => amount)
* max_amt - max capacity (number for a single bin, list of (material_id => max_amount) for per-mat bins)
* show_on_examine - If t
*/
/datum/component/material_container/Initialize(list/mat_list, max_amt = 0, _show_on_examine = FALSE, list/allowed_types, datum/callback/_precondition, datum/callback/_after_insert, _disable_attackby)
	...
	// Sanity check of materials list
	if(!LAZYLEN(materials))
		log_debug("[src] initialized with no allowed material types.")
		materials = list()
	for(var/ID in materials)
		var/datum/material/M = get_material_by_name(ID)
		if(!istype(M))
			log_debug("[src] initialized with invalid material [ID]")
			materials -= ID
		if(!(materials[ID] >= 0)) // Catch any non-numeric or negative values
			materials[ID] = 0
...
/datum/component/material_container/proc/OnExamine(datum/source, mob/user, list/examine_list)
	if(show_on_examine)
		for(var/ID in materials)
			var/datum/material/M = get_material_by_name(ID)
			var/amt = materials[ID]
			if(amt)
				examine_list += "<span class='notice'>It has [amt] unit\s of [lowertext(M.display_name)] stored.</span>"
...
/datum/component/material_container/proc/OnAttackBy(datum/source, obj/item/I, mob/living/user)
	...
	// TODO - Generic way of exclusing holographic items.
	// TODO - Generic way of prohibiting items from material redemption
...
//
// API Procs - For interacting with material storage
//


//For inserting an amount of material. If ID is null, it inserts that amount of *every* material it contains.
