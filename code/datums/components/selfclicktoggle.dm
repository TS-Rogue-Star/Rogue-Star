...
/datum/component/selfclicktoggle/Initialize()
	...
	parentmob.verbs += /mob/proc/toggle_clickthroughself
...
/datum/component/selfclicktoggle/proc/_clickthrough_unregister_signals()
	UnregisterSignal(parent, list(COMSIG_ATOM_DIR_CHANGE, COMSIG_ATOM_UPDATE_OVERLAYS, COMSIG_MOB_LOGOUT, COMSIG_PARENT_QDELETING))
...
/mob/proc/toggle_clickthroughself()
	...
	var/datum/component/selfclicktoggle/transparency = GetComponent(/datum/component/selfclicktoggle)
	transparency.do_the_thing()

/mob/Logout() // For some gods-forsaken reason, SEND_SIGNAL does not work in /mob/Logout(). Words cannot express my fucking confusion.
	. = ..()
	var/datum/component/selfclicktoggle/transparency = GetComponent(/datum/component/selfclicktoggle)
	if(!isnull(transparency))
		transparency._clickthrough_on_logout()

/mob/Login()
	. = ..()
	LoadComponent(/datum/component/selfclicktoggle)
