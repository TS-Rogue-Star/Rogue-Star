...
/obj/machinery/alarm/tgui_data(mob/user, datum/tgui/ui, datum/tgui_state/state)
	var/list/data = list(
		"locked" = locked,
		"siliconUser" = siliconaccess(user) || (isobserver(user) && is_admin(user)), //RS Port Chomp PR 8077 | CHOMPEdit borg access + admin access
	...
	if(!locked || siliconaccess(user) || data["remoteUser"] || (isobserver(user) && is_admin(user))) //RS Port Chomp PR 8077 | CHOMPEdit borg access + admin access
...
/obj/machinery/alarm/tgui_act(action, params, datum/tgui/ui, datum/tgui_state/state)
	...
	if((locked && !(siliconaccess(usr) || (isobserver(usr) && is_admin(usr))) && !istype(state, /datum/tgui_state/air_alarm_remote)) || (issilicon(usr) && aidisabled)) //RS Port Chomp PR 8077 | CHOMPedit borg access
		return

	var/device_id = params["id_tag"]
	switch(action)
		if("lock")
			if((siliconaccess(usr) && !wires.is_cut(WIRE_IDSCAN)) || (isobserver(usr) && is_admin(usr))) //RS Port Chomp PR 8077 | CHOMPEdit borg access + admin acces
				locked = !locked
				. = TRUE
		if( "power",
			"o2_scrub",
