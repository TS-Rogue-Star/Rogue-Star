...
/obj/machinery/hologram/holopad/proc/activate_holo(mob/living/silicon/ai/user)
	if(powered() && user.eyeobj && user.eyeobj.loc == src.loc)//If the projector has power and client eye is on it
		if (user.holo)
...
/obj/machinery/hologram/holopad/proc/link_pda(obj/item/device/pda/pda)
	if (!istype(pda))
		return
...
/obj/machinery/hologram/holopad/proc/unlink_pda(obj/item/device/pda/pda)
...
/obj/machinery/hologram/holopad/hear_talk(mob/living/M, text, verb, datum/language/speaking)
	if(M)
		for(var/mob/living/silicon/ai/master in masters)
			...
			// Commenting out short lengths, since we don't use those on RS. - Enem
			...
			var/prefix = "<a href='byond://?src=\ref[master];trackname=[html_encode(name_used)];track=\ref[M]'>Follow</a>" // No need to use a var here now. -Enem
...
/obj/machinery/hologram/holopad/proc/get_hear_message(name_used, text, verb, datum/language/speaking, prefix = "")
	if(speaking)
		return "<i>[span("game say", "Holopad received, [span("name", name_used)][prefix] [speaking.format_message(text, verb)]")]</i>"
	return "<i>[span("game say", "Holopad received, [span("name", name_used)][prefix] [verb], [span("message", text)]")]</i>"
...
/obj/machinery/hologram/holopad/proc/set_dir_hologram(new_dir, mob/living/silicon/ai/user)
	if(masters[user])
		var/obj/effect/overlay/aiholo/hologram = masters[user]
...
/obj/machinery/hologram/holopad/Destroy()
	...
	if (LAZYLEN(linked_pdas))
		for (var/obj/item/device/pda/pda as anything in linked_pdas)
			unlink_pda(pda)
		linked_pdas = null
...
/mob/verb/hologram_test()
	...
	var/obj/effect/overlay/aiholo/hologram = new(loc)//Spawn a blank effect at the location.
