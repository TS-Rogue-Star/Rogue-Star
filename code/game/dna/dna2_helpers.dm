...
/mob/proc/UpdateAppearance(var/list/UI=null)
	if(istype(src, /mob/living/carbon/human))
		...
		var/ears_secondary = dna.GetUIValueRange(DNA_UI_EAR_SECONDARY_STYLE, ear_styles_list.len + 1) - 1 // RS EDIT START (Port of VS PR#16513 'Adds a second ear slot.')
		if(ears_secondary < 1)
			H.ear_secondary_style = null
		else if((0 < ears_secondary) && (ears_secondary <= ear_styles_list.len))
			H.ear_secondary_style = ear_styles_list[ear_styles_list[ears_secondary]] // RS EDIT END (Port of VS PR#16513 'Adds a second ear slot.')
		...
		LAZYINITLIST(H.ear_secondary_colors) // RS EDIT START (Port of VS PR#16513 'Adds a second ear slot.')
